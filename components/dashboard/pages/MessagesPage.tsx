...
import { Search, Menu, Check, Inbox, ArrowLeft, Video, MoreVertical, Play, MapPin, Image, Mic, Send, CheckCheck, X } from "lucide-react";
...
export default function MessagesPage() {
    ...
    const [modalContent, setModalContent] = useState<{ type: 'video' | 'image', src: string, duration?: string } | null>(null);

    const handleBackClick = () => {
        setSelectedChat(null);
    };

    const handleMediaClick = (type: 'video' | 'image', src: string, duration?: string) => {
    ...
    const renderMessage = (message: Message) => {
                    ...
                    {/* Short Time Card */}
                     <div className="bg-gray-800 rounded-[20px] p-4 w-[317px] h-[180px] flex flex-col justify-between">
                         <div className="space-y-3">
                             <div className="flex justify-between items-center">
                                 ...
                                 <span className="text-white font-semibold text-[20px]">50,000.00 APH</span>
                             </div>
                             <div className="flex justify-between items-center">
                                 <span className="text-white text-[20px]">Outcall</span>
                                 <span className="text-white font-semibold text-[20px]">70,000.00 APH</span>
                             </div>
                         </div>
                     ...
                     <div className="bg-gray-800 rounded-[20px] p-4 w-[317px] h-[180px] flex flex-col justify-between">
                         <div className="space-y-3">
                             ...
                             <div className="flex justify-between items-center">
                                 <span className="text-white text-[20px]">Outcall</span>
                                 <span className="text-white font-semibold text-[20px]">100,000.00 APH</span>
                             </div>
                         </div>
                         <button className="w-full bg-[#FA266D] text-white py-2 px-4 rounded-[30px] text-[20px] font-medium">
                             Book overnight
        ...
        if (message.type === 'video') {
            return (
                <div className="bg-white rounded-lg p-2 max-w-xs">
                    <div className="relative cursor-pointer" onClick={() => handleMediaClick('video', message.videoThumbnail || '', message.duration)}>
        ...
        if (message.type === 'image') {
            return (
                <div className="bg-white rounded-lg p-2 max-w-xs">
                    <div className="relative cursor-pointer" onClick={() => handleMediaClick('image', message.videoThumbnail || '')}>
                        <img
        ...
        return (
            <div className={`rounded-lg p-3 max-w-xs ${message.sender === 'me'
                    ? 'bg-white text-gray-800'
                    : 'bg-[#FA266D] text-white'
                }`}>
                <p className="text-sm">{message.content}</p>
                <div className={`flex items-center justify-end gap-1 mt-1 ${message.sender === 'me' ? 'text-gray-500' : 'text-pink-100'
                    }`}>
                ...
                <div className="flex-1 overflow-y-auto scrollbar-hide">
                    {mockChats.map((chat) => (
                        <div
                            ...
                            className={`p-4 border-b border-white/5 cursor-pointer hover:bg-white/5 transition-colors ${selectedChat === chat.id ? 'bg-white/10' : ''
                                }`}
                        >
                            ...
                            <div className="space-y-3">
                                ...
                                />
                                 ...
                                 <div className="flex items-center justify-end gap-3">
                                     {/* Action Buttons */}
                                     <button className="text-[#FA266D] hover:text-pink-400 flex items-center gap-2 text-sm">
